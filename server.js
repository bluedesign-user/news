...
// RSS feed sources for construction/architecture news
const RSS_FEEDS = {
  general: {
    name: '建設・建築 総合',
...
// Sample data used when RSS feeds are unavailable (e.g., no internet)
const SAMPLE_DATA = {
  general: [
...
async function fetchRSS(feedKey) {
  ...
  try {
    ...
    cache[feedKey] = { data: articles, timestamp: Date.now() };
...
// API: Get list of available categories
...
// API: Get news for a specific category
...
// API: Get all news from all categories
